...
\item Should the manipulators be in header \tcode{<osyncstream>} instead of globally available in \tcode{<ostream>} as are \tcode{flush} and \tcode{endl}?
...
\item Should the \tcode{Allocator} be part of the stream type (LWG in Toronto: \textbf{YES}), or should it simply use \tcode{polymorphic_allocator<CharT>}, consistent with P0339? Pablo thinks the latter would be easier to use. LWG: keep like in the paper, polymorphic allocators should be applied throughout.
...
\pnum
\begin{remarks}
This assignment operator disassociates \tcode{rhs} from its wrapped stream buffer ensuring destruction of \tcode{other} produces no output. 
\end{remarks}
\end{itemdescr}
...
\pnum
\postconditions
The value returned by \tcode{get_wrapped()} is the value returned by \tcode{os.get_wrapped()} prior to calling this constructor. \tcode{nullptr == os.get_wrapped()}.
\end{itemdescr}
...
If an exception is thrown from \tcode{emit()}, that exception is caught and ignored.
Move assigns \tcode{sb} from \tcode{other.sb}.
\begin{note}
