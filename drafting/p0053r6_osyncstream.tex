...
\item Should the \tcode{Allocator} be part of the stream type, or should it simply use \tcode{polymorphic_allocator<CharT>}, consistent with P0339? Pablo thinks the latter would be easier to use.
...
\pnum
\effects
\tcode{flush(os)}, if \tcode{os} is a \tcode{basic_osyncstream<charT, traits, Allocator>} reference,  \tcode{dynamic_cast<basic_osyncbuf<charT, traits, Allocator>*>(os.rdbuf())->emit(),}. 
\pnum
\returns
\tcode{os}.
\end{itemdescr}
