...
\begin{itemdescr}
\pnum
\remark This constructor only takes part in overload resolution if \tcode{is_constructible_v<EF,EFP>}.

\pnum
\requires The call expression of \tcode{EF} shall not throw an exception.

\pnum
\effects If \tcode{EFP} is not a lvalue-reference type, move-constructs  \tcode{exit_function} from \tcode{f}  if that can not throw an exception, otherwise copy-constructs \tcode{exit_function} from \tcode{f}. If the copying throws an exception, calls \tcode{f()}.
...
\pnum
\effects \effects If \tcode{EFP} is not a lvalue-reference type, move-constructs  \tcode{exit_function} from \tcode{f} if that can not throw an exception, otherwise copy-constructs \tcode{exit_function} from \tcode{f}.\\
\enternote
...
\tcode{d(r)} shall be well formed, shall have well-defined behavior, and does not throw an exception.
...
using @\tcode{\textit{_is_constructible}}@ =
     conditional_t<
        is_reference_v<TT> || !is_nothrow_move_constructible_v<TT>,
        is_constructible<T, TT const &>,
        is_constructible<T, TT>>;
template<class T, class TT>
constexpr auto @\tcode{\textit{is_copy_or_nothrow_move_constructible_from_v}}@=
		@\tcode{\textit{_is_constructible}}@<T,TT>::value;
\end{codeblock}
this constructor shall not participate in overload resolution unless
\\ \tcode{\textit{is_nothrow_move_or_copy_constructible_from_v}<R, RR>}\\
and\\
\tcode{\textit{is_nothrow_move_or_copy_constructible_from_v}<D, DD>}

\pnum
\effects If RR is not a lvalue-reference, move-constructs \tcode{resource} from \tcode{r} if that can not throw an exception, otherwise copy-constructs \tcode{resource} from \tcode{r}. Then, if \tcode{DD} is not an lvalue reference, move-constructs \tcode{deleter} from \tcode{d} if that can not throw an exception, otherwise copy-constructs \tcode{deleter} from \tcode{d}. If construction of \tcode{resource} throws an exception, calls \tcode{d(r)}.  If construction of \tcode{deleter} throws an exception, calls \tcode{d(resource)}. 
\enternote
...
\begin{itemdescr}
\pnum
\effects Move-constructs \tcode{resource} from \tcode{rhs.resource} if that can not throw an exception, otherwise copy-constructs \tcode{resource} from \tcode{rhs.resource}.  
\enternote
If construction of \tcode{resource} throws an exception \tcode{rhs} is left owning the resource and will free it in due time.
%If construction of \tcode{resource} throws an exception, leaves \tcode{rhs} in its state. 
\exitnote
Then move-constructs \tcode{deleter} from \tcode{rhs.deleter} if that can not throw an exception, otherwise copy-constructs \tcode{deleter} from \tcode{rhs.deleter}. If construction of \tcode{deleter} throws an exception: if \tcode{!is_nothrow_move_constructible_v<R>}, then \tcode{rhs.deleter(resource) ; rhs.release();} otherwise \tcode{rhs.resource} and \tcode{rhs.deleter} are unmodified and \tcode{rhs} can be left owning the resource. 
