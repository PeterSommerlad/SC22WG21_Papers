...
\textit{Note that p0407 provides allocator support for \tcode{basic_stringbuf}, depending on acceptance sequence of the papers, some overloads need to be further added/adjusted and the wording also adjusted. At the time of p0408r3 that was forwarded to LWG, p0407 was not yet reviewed by LEWG.}
\begin{codeblock}
...
%old:
Then calls \tcode{str(std::move(s))}.
%% p0407: 
%\added{, and initializing \tcode{buf} with \tcode{\{std::move(s)\}}}.
\end{itemdescr}
\end{insrt}
