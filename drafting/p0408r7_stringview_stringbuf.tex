...
\pnum
\effects
%% could upgrade the effects clause like with spanstream, but won't
%Equivalent to:
%\begin{codeblock}
%    basic_stringbuf tmp{std::move(rhs)};
%    this->swap(tmp); // would mean noexcept could be different, because of allocator propagation
%    return *this;
%\end{codeblock}
...
\begin{addedblock}
\pnum
\expects \tcode{allocator_traits<Allocator>::propagate_on_container_swap::value} is \tcode{true} or \tcode{get_allocator() == s.get_allocator()} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock} 
\effects Equivalent to: \tcode{return basic_string<charT, traits, Allocator>(view(), get_allocator());}
\end{addedblock}
\end{itemdescr}
...
\pnum
\effects Equivalent to: \tcode{return basic_string<charT, traits, SAlloc>(view(), sa);}
\end{itemdescr}
\end{addedblock}
...
\pnum
\ensures The underlying character sequence \tcode{buf} is empty and \tcode{pbase()}, \tcode{pptr()}, \tcode{epptr()}, \tcode{eback()}, \tcode{gptr()}, \tcode{egptr()} are initialized as if by calling \tcode{init_buf_ptrs()} with an empty \tcode{buf}.
\end{itemdescr}
...
\pnum
\returns A \tcode{sv} object referring to the \tcode{basic_stringbuf}'s underlying character sequence
in \tcode{buf}:
\begin{itemize}
\item If \tcode{ios_base::out} is set in \tcode{mode},
...
\pnum
\begin{note}
Using the returned \tcode{sv} object after destruction or invalidation of the character sequence underlying \tcode{*this} is undefined behavior, unless \tcode{sv.empty()} is \tcode{true}. 
\end{note}
\end{itemdescr}
\end{addedblock}
...
\end{codeblock} % no noexcept possible because base class does not have it.
...
\textit{change the definition of the swap member to incorporate the new requirement implicitly given by \tcode{basic_stringbuf::swap}:}
...
\begin{itemdescr}
\pnum
\effects \removed{Exchanges the state of \tcode{*this} and
\tcode{rhs} by calling}
\added{Equivalent to:}\\
\tcode{basic_istream<charT, traits>::swap(rhs)}\added{\tcode{;}}\removed{ and}\\
\tcode{sb.swap(rhs.sb)}\added{\tcode{;}}\removed{.}
\end{itemdescr}
...
\tcode{rhs} by calling}
\added{Equivalent to:}\\
\tcode{basic_ostream<charT, traits>::swap(rhs)}\added{\tcode{;}}\removed{ and}\\
\tcode{sb.swap(rhs.sb)}\added{\tcode{;}}\removed{.}
\end{itemdescr}
...
\tcode{rhs} by calling}
\added{Equivalent to:}\\
\tcode{basic_iostream<charT, traits>::swap(rhs)}\added{\tcode{;}}\removed{ and}\\
\tcode{sb.swap(rhs.sb)}\added{\tcode{;}}\removed{.}
\end{itemdescr}
