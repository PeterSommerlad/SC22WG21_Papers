...
\tcode{make_unique_resource()} and  \tcode{make_unique_resource_checked()} to create their instances. The usage of the RAII (resource acquisition is initialization) wrappers assumes that the resource type and the exit functions/deleter provided do not throw exceptions and that they are \tcode{nothrow_copy_constructible}.
\enternote
...
\pnum
\enternote
\tcode{scope_exit} is meant to be a universal scope guard to call its exit function on scope exit.
\exitnote
...
\pnum
\requires \tcode{EF} shall be a CopyConstructible (Table~21
...
\pnum
If the exit function type \tcode{EF} shall satisfy
...
\begin{itemdescr}
\pnum
\effects \tcode{execute_on_destruction_flag=false}.
...
\pnum
\enternote
These factory functions allow declarative control flow when used with lambdas as arguments if their return value is stored in a local variable. An exception thrown from \tcode{exit_function} when called from \tcode{scope_exit}'s destructor causes the program to \tcode{terminate()}.
\exitnote
...
\pnum
\tcode{R} shall be a CopyConstructible and CopyAssignable.
...
%Invokes the deleter function for resource if it was not previously released, e.g. \tcode{ this->reset(); }  Then moves newresource into the tracked resource member, e.g. \tcode{this->resource = std::move(newresource);}  Finally sets the object in the non-released state so that the deleter function will be invoked on destruction if \tcode{release()} is not called first.
...
%\rSec2[scope.unique_resource.make_unique_resource]{Factories \tcode{unique_resource}}
\subsection {Factories for \tcode{unique_resource} [scope.unique_resource.make_unique_resource]}
\begin{itemdecl}
template<typename R,typename D>
unique_resource<R,D>
make_unique_resource( R r, D d) noexcept;
...
\pnum
\requires \tcode{R} and \tcode{S} shall be EqualityComparable with each other.
...
auto
make_unique_resource_checked(R r, S invalid, D d ) noexcept {
  bool mustrelease = not bool(r == invalid);
  auto ur= unique_resource<R,D>(r, d);
  if(mustrelease) ur.release();
  return ur;
}
