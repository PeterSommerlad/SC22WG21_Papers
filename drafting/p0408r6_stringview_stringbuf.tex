...
\item is \tcode{!is_same_v<SAlloc,Allocator>} the correct SFINAE predicate.
\item \tcode{str()} member functions taking an allocator argument to return a different string type.
\item \tcode{noexcept} for \tcode{view()} member function (suggested by LWG).
\item does an Allocator template parameter that is mapped to \tcode{basic_string}'s Allocator template parameter need to conform to \oldconcept{Allocator} requirements.
\end{itemize}
...
\item make the setter \tcode{str()} overload a template member copying into the string buffer to take an allocator template parameter that could differ from the buffer's own \tcode{Allocator} 
...
\begin{codeblock}
    ...
    void str(const basic_string<charT, traits, @\removed{Allocator}\added{SAlloc}@>& s);
\end{codeblock}
\begin{addedblock}
\begin{codeblock}
...
\pnum
\mandates \tcode{!is_same_v<SAlloc,Allocator>}.
%\tcode{SAlloc} shall satisfy the \oldconcept{Allocator} requirements
%(\tref{utilities.allocator.requirements}).
%% TODO
...
\begin{itemdescr}
\pnum
\effects \added{Move assigns \tcode{buf} from \tcode{std::move(rhs).str()}.}
