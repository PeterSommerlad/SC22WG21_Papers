...
\tcode{mode}
with \tcode{which} \added{, and \tcode{buf} with \tcode{s}. Calls  \tcode{init_buf_ptrs()}}.
...
\tcode{mode}
with \tcode{which}, and \tcode{buf} with \tcode{a}. Calls \tcode{init_buf_ptrs()}.
...
\begin{itemdescr}
\pnum
\effects Constructs an object of class \tcode{basic_stringbuf}, initializing the base class with \tcode{basic_streambuf()} \iref{streambuf.cons}, initializing \tcode{mode} with \tcode{which}, and  \tcode{buf} with \tcode{std::move(s)}. Calls \tcode{init\_buf\_ptrs()}.
\end{itemdescr}
...
\pnum
\effects Constructs an object of class \tcode{basic_stringbuf}, initializing the base class with \tcode{basic_streambuf()} \iref{streambuf.cons}, initializing \tcode{mode} with \tcode{which}, and  \tcode{buf} with \tcode{\{s,a\}}. Calls \tcode{init\_buf\_ptrs()}.
...
\pnum
\constraints \tcode{!is_same_v<SAlloc,Allocator>}.
%\tcode{SAlloc} shall satisfy the \oldconcept{Allocator} requirements
%(\tref{utilities.allocator.requirements}).
%% TODO

\pnum
\effects Constructs an object of class \tcode{basic_stringbuf}, initializing the base class with \tcode{basic_streambuf()} \iref{streambuf.cons}, initializing \tcode{mode} with \tcode{which}, and  \tcode{buf} with \tcode{s}. Calls \tcode{init\_buf\_ptrs()}.
...
\emph{Note to LWG reviewers: using std::move(rhs).str() ensures rhs.buf is in a consistent state before the move happens. Before the spec was wrong, because rhs.buf might have been shorter than the actual written characters.}

\begin{itemdescr}
\pnum
\effects Move constructs from the rvalue \tcode{rhs}. \added{In the first form \tcode{buf} is initialized from \tcode{std::move(rhs).str()}. In the second form \tcode{buf} is initialized from \tcode{\{std::move(rhs).str(), a\}}.}
It is
...
the \tcode{basic_stringbuf} with a \tcode{basic_string}, or by calling one of 
the
\tcode{str(basic_string)} setter member functions. In the case of calling 
one of 
the
\tcode{str(basic_string)} setter member functions, all characters initialized prior to
...
\pnum
\begin{note}
For efficiency reasons stream buffer operations might violate invariants of \tcode{buf} while it is held encapsulated in the \tcode{basic_stringbuf}, i.e., by writing to characters in the range [\tcode{buf.data()+buf.size(), buf.data()+buf.capacity()}). All operations retrieving a \tcode{basic_string} from \tcode{buf} ensure that the \tcode{basic_string} invariants hold on the returned value. Implementations might make \tcode{basic_stringbuf} a friend class to \tcode{basic_string} to achieve that.
\end{note}
\end{itemdescr}
\end{addedblock}
...
\begin{itemdecl}
    ...
    @\added{basic_string<charT, traits, SAlloc> str(const SAlloc\& sa) const ;}@

\end{itemdecl}

\textit{Change p1 (now p2) to use plural for "\tcode{str(basic_string)} member functions" and refer to the allocator. Constraints on SAlloc were taken from [container.requirements.general] p.17:}
...
\tcode{basic_stringbuf}\added{'s}
underlying character sequence.
\added{In the second form the \tcode{basic}\_\tcode{string<charT, traits, SAlloc>} is constructed with allocator \tcode{sa}.}
...
contains the character sequence in the range \range{pbase()}{high_mark}\removed{, where
\tcode{high_mark} represents the position one past the highest initialized character
in the buffer. Characters can be initialized by writing to the stream, by constructing
the \tcode{basic_stringbuf} with a \tcode{basic_string}, or by calling 
the
\tcode{str(basic_string)} member function. In the case of calling 
the
...
\pnum 
\constraints 
%The second form does not participate in overload resolution, unless \tcode{SAlloc} satisfies both of the following conditions: 
\begin{itemize}
...
in \tcode{buf}. 
This can be achieved by first adjusting \tcode{buf} to the underlying character sequence as follows.

If the \tcode{basic_stringbuf} was created only in input mode, adjust \tcode{buf} to contain
\tcode{basic_string(eback(), egptr()-eback())}. 
If the \tcode{basic_stringbuf} was created with \tcode{which \& ios_base::out} being true 
then adjust \tcode{buf} to contain \tcode{basic_string(pbase(), high_mark-pbase())}.
Otherwise the \tcode{basic_stringbuf} has been created in neither input nor output mode 
and a zero length \tcode{basic_string} is returned. 

\pnum
\begin{note}
The explained effect can be achieved by setting \tcode{buf}'s \tcode{size()} to the resulting length (\tcode{egptr()-eback()} or \tcode{high_mark-pbase()}) and returning \tcode{std::move(buf)}. However, \tcode{basic_string} does not provide setting the length without changing the characters in the buffer in the range \tcode{[buf.data()+buf.size(), buf.data()+buf.capacity())} as a public member function.
\end{note}
...
%% LWG San Diego (Alisdair & Jonathan Wakely)
%\pnum
%\begin{note}
...
\pnum
\begin{note}
Using the returned \tcode{basic_string_view} object after destruction or any modification of the character sequence underlying \tcode{*this}, such as output on the holding stream, will cause undefined behavior, because the internal string referred to by the return value might have changed or been re-allocated. 
\end{note}
\end{itemdescr}
\end{addedblock}
...
\pnum
\effects 
\tcode{buf = std::move(s); init_buf_ptrs()}.
\end{itemdescr}
\end{insrt}
...
void str(const basic_string<charT, traits, Allocator>& s);
@\added{template<class SAlloc>}@
@\added{void str(const basic_string<charT, traits, SAlloc>\& s);}@
\end{itemdecl}
...
\pnum
\effects
\added{\tcode{buf = s; init}_\tcode{buf}_\tcode{ptrs()}}.
% note: this takes into account that a basic_string can be assigned from a basic_string_view and 
% s can always be converted to one.
\removed{
...
\tcode{basic_stringbuf<charT, traits, Allocator>(which | ios_base::in)}\linebreak
\iref{stringbuf.cons}.
\end{itemdescr}
...
\tcode{basic_istream<charT, traits>(addressof(sb))}\iref{istream}
and initializing \tcode{sb} with%\linebreak % avoid Overfull
\tcode{basic_stringbuf<charT, traits, Allocator>(str, which | ios_base::in)}
\iref{stringbuf.cons}.
\end{itemdescr}
...
and initializing \tcode{sb} with %\linebreak % avoid Overfull
\tcode{basic_stringbuf<charT, traits, Allocator>(which | ios_base::out)}\linebreak
\iref{stringbuf.cons}.
\end{itemdescr}
...
\tcode{basic_ostream<charT, traits>(addressof(sb))}\iref{ostream}
and initializing \tcode{sb} with%\linebreak % avoid Overfull
\tcode{basic_stringbuf<charT, traits, Allocator>(str, which | ios_base::out)}
\iref{stringbuf.cons}.
\end{itemdescr}
...
\tcode{basic_ostream<charT, traits>(addressof(sb))}\iref{ostream}
and initializing \tcode{sb} with
\tcode{basic_stringbuf<charT, traits, Allocator>(std::move(s), which | ios_base::out)}
\iref{stringbuf.cons}.
\end{itemdescr}
...
\tcode{basic_iostream<charT, traits>(addressof(sb))}\iref{iostream.cons}
and initializing \tcode{sb} with%\linebreak % avoid Overfull
\tcode{basic_stringbuf<charT, traits, Allocator>(str, which)}
\iref{stringbuf.cons}.
\end{itemdescr}
...
\effects
Constructs an object of class
\tcode{basic_stringstream<charT, traits, Allocator>},
initializing the base class with
\tcode{basic_iostream<charT, traits>(addressof(sb))}\iref{iostream.cons}
and initializing \tcode{sb} with
...
\tcode{basic_iostream<charT, traits>(addressof(sb))}\iref{iostream.cons}
and initializing \tcode{sb} with
\tcode{basic_stringbuf<charT, traits, Allocator>(std::move(s), which)}
\iref{stringbuf.cons}.
\end{itemdescr}
