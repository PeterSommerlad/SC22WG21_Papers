...
\added{This paper indirectly addresses CH 9 "... and more".}
...
Change the \tcode{const}-overload of \tcode{str()} member function to add a reference qualification. This avoids ambiguities with the rvalue-ref overload of \tcode{str()}. Add two overloads of the \tcode{str()} member function and add the \tcode{str_view()} member function.
...
Change the \tcode{const}-overload of \tcode{str()} member function to add a reference qualification. This avoids ambiguities with the rvalue-ref overload of \tcode{str()}. Add an overload of the \tcode{str()} member function and add the \tcode{str_view()} member function:
