...
For more classic code with Base classes with a virtual destructor, this proposal introduces safe_delete deleter, that is limiting a u\tcode{nique_ptr<Base,safe_delete<Base>>} move of a \tcode{unique_ptr<Derived,safe_delete<Derived>>} if \tcode{Base} has a virtual destructor.
...
\end{itemdecl}
\pnum
\effects This constructor is only available, when either \tcode{T} and \tcode{U} are the same or \tcode{U*} is convertible to \tcode{T*} and \tcode{T} provides a virtual destructor.
\\
\pnum
\enternote
