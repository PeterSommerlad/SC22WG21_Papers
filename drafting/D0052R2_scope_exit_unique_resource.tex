...
\begin{itemdescr}
\pnum
\effects Constructs a \tcode{scope_fail} object that will call \tcode{f()} on its destruction if its scope is left with a new exception, unless \tcode{release()} was called prior to that.
\end{itemdescr}
...
\end{codeblock}
If move-assignment of the resource fails, \tcode{get_deleter()(r)} on the original value of r.
