...
\begin{itemize}
\item Should we make the regular constructor of the scope guard templates private and friend the factory function only? This could prohibit the use as class members, which might sneakily be used to create "destructor" functionality by not writing a destructor.
...
\item Are the exception specifications correct? They are derived from Eric Niebler's implementation and should be cross checked (Daniel Kr\"ugler did, but I am not sure, I adjusted them correctly).
\item Should we provide a non-explicit conversion operator to R in unique_resource<R,D> ? Last time people seem to have been strongly against, however, it would make the use of unique_resource much easier in contexts envisioned by author Andrew Sandoval. Please re-visit, since it is omitted here.
\item LWG: Do we need to specifiy the \tcode{unique_resource<R\&,D>} lvalue reference specialization, that is required to implement assignment etc.
\end{itemize}
...
\pnum
\effects For the first form, initializes \tcode{exit_function} with \tcode{f}, for the second form, initializes \tcode{exit_function} with \tcode{forward<EF>(f)}. 
If construction fails, calls \tcode{f()}.
...
\tcode{execute_on_destruction} yields the value \tcode{rhs.execute_on_destruction} yielded before the construction and sets \tcode{rhs.execute_on_destruction} to \tcode{false}.
If \tcode{is_nothrow_move_constructible_v<EF>} move constructs otherwise copy constructs \tcode{exit_function} from \tcode{rhs.exit_function}.  In case of an exception during the last operations, calls \tcode{rhs.exit_function()} if it would be called when \tcode{rhs} would have been destroyed without being moved from.
\end{itemdescr}
...
% or an lvalue reference, which will be wrapped with a reference_wrapper<R> internally.%%need to disucss
...
\begin{itemdecl}
    ...
    explicit unique_resource(RR &&r, DD &&d)
        noexcept((is_nothrow_constructible_v<R, RR> 
                  || is_nothrow_constructible_v<R, const R &>)
              && (is_nothrow_constructible_v<D, DD> 
                  || is_nothrow_constructible_v<D, const D &>))
\end{itemdecl}
...
\\ \tcode{is_nothrow_move_or_copy_constructible_from_v<R, const RR \&>}\\
and\\
\tcode{is_nothrow_move_or_copy_constructible_from_v<D, const DD \&>}
...
\end{codeblock}
If move-assignment of \tcode{resource} throws an exception, \tcode{get_deleter()(r)} on the original value of \tcode{r} and \tcode{execute_on_destruction=false}.
\end{itemdescr}
...
\begin{codeblock}
  ...
  execute_on_destruction = true;
\end{codeblock}
If copy-assignment of \tcode{resource} fails, \tcode{get_deleter()(r)} and \tcode{execute_on_destruction=false}.
\end{itemdescr}
...
\pnum \enternote There is no need to overload on \tcode{reference_wrapper} for the deleter. A specialization for \tcode{unique_resource<R\&,D>} should internally re-wrap the reference obtained through \tcode{r.get()} \exitnote
\end{itemdescr}
