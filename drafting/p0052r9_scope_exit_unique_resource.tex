...
\newcommand{\papernumber}{d0052r10}
...
\begin{itemize}
\item Added default constructor as proposed by p1411, to ease integration, once LEWG blesses it. This will be shown in \added{green}.
\end{itemize}
...
\pnum
\requires
%\mandates %% old concepts should stay requires.
...
the requirements of \oldconcept{Destructible} (Table~27
%\ref{destructible}
). 
...
The expression \tcode{f()} shall be well-formed.

\pnum
\expects
Calling \tcode{f()} shall have well-defined behavior and shall not throw an exception.
...
\pnum
\expects
Calling \tcode{f()} shall have well-defined behavior.
...
\begin{itemize}
 \item
if \tcode{is_nothrow_move_constructible_v<EF>} is true, \tcode{EF} shall meet the requirements of \oldconcept{MoveConstructible} (Table 23), 
\item
otherwise \tcode{EF} shall meet the requirements of \oldconcept{CopyConstructible} (Table 24).
\end{itemize}
...
\end{addedblock}
\begin{codeblock}
  @\added{unique_resource(); // \seebelow }@ 
\end{codeblock}
\begin{codeblock}
...
\tcode{D} shall either meet the \oldconcept{CopyConstructible} requirements (Table 24
%\ref{copyconstructible}
), or 
\tcode{D} shall meet the \oldconcept{MoveConstructible} requirements (Table 23
...
\oldconcept{CopyConstructible} (Table 24
%\ref{copyconstructible}
) requirements,  or 
is an object type that meets the \oldconcept{MoveConstructible} (Table 23
%\ref{moveconstructible}
) requirements and
...
\pnum
\effects
Constructs a \tcode{unique_resource} where \tcode{resource} and \tcode{deleter} are value-initialized and \tcode{execute_on_reset} is initialized with \tcode{false}.
\end{itemdescr}
\end{addedblock}
...
\pnum
%\requires % not mandates
%The expression
%\tcode{d(r)} shall be well-formed and shall have well-defined behavior.
%The expressions \tcode{d(\textit{RESOURCE})} and \tcode{deleter(\textit{RESOURCE})} shall be well-formed and shall have well-defined behavior.
% alt:
\mandates
The expression
\tcode{d(r)}, \tcode{d(\textit{RESOURCE})} and \tcode{deleter(\textit{RESOURCE})} shall be well-formed.

\pnum
\expects
Calling \tcode{d(r)}, \tcode{d(\textit{RESOURCE})} or \tcode{deleter(\textit{RESOURCE})} shall have well-defined behavior and shall not throw an exception. 
...
\begin{codeblock}
    rhs.deleter(@\textit{RESOURCE}@); rhs.release();
\end{codeblock}
...
If \tcode{is_nothrow_move_assignable_v<R1>} is \tcode{true}, \tcode{R1} shall meet the \oldconcept{MoveAssignable} requirements (Table 25), otherwise \tcode{R1} shall meet the \oldconcept{CopyAssignable} (Table 26) requirements.
If \tcode{is_nothrow_move_assignable_v<D>} is \tcode{true}, \tcode{D} shall meet the \oldconcept{MoveAssignable} requirements (Table 25), otherwise \tcode{D} shall meet the \oldconcept{CopyAssignable} requirements (Table 26).
%\requires %\mandates not useful for oldconcept
%If \tcode{is_nothrow_move_assignable_v<R1>} is \tcode{true}, \tcode{R1} shall be \oldconcept{MoveAssignable} (Table 25), otherwise \tcode{R1} shall be \oldconcept{CopyAssignable} (Table 26).
%If \tcode{is_nothrow_move_assignable_v<D>} is \tcode{true}, \tcode{D} shall be \oldconcept{MoveAssignable} (Table 25), otherwise \tcode{D} shall be \oldconcept{CopyAssignable} (Table 26).
%\mandates
...
\pnum
\mandates
The expression \tcode{deleter(r)} shall be well-formed.

\pnum
\expects
Calling \tcode{deleter(r)} shall have well-defined behavior and shall not throw an exception.
...
\begin{itemdescr}
\pnum
\mandates The expression \tcode{( resource == invalid ? true : false )} shall be well-formed.

\pnum
\expects Evaluation of the expression \tcode{( resource == invalid ? true : false )} shall have well-defined behavior and shall not throw an exception.
