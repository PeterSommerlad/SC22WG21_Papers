...
\effect
A valid \CppXVII{} program that relies on the deprecated \tcode{char *} stream classes may fail to compile. Such a program must be adapted to employ classes and functions from [span.streams] or [string.streams] to achieve its functionality with equivalent performance.

\subsection{Remove deprecated \tcode{char *} streams from Appendix D}
