...
\pnum
\added{Except where explicitly specified,} 
...
\pnum
The following clauses describe the class templates \tcode{scope_exit}, \tcode{scope_fail}, and \tcode{scope_success}. In each clause, the name \tcode{\textit{scope_guard}} denotes either of these class templates. In description of the class members \tcode{\textit{scope_guard}} refers to the enclosing class.
...
template <class EF>
@\tcode{\textit{scope_guard}}@(EF &&ef) -> @\tcode{\textit{scope_guard}}@<std::decay_t<EF>>;
...
\pnum
\requires Given an lvalue \tcode{f} of type \tcode{remove_reference_t<EFP>}, the expression \tcode{f()} shall be well formed, have well-defined behavior, and not throw an exception.


\pnum
\effects If \tcode{EFP} is not an lvalue-reference type and \tcode{is_nothrow_constructible_v<EF,EFP>} is \tcode{true}, initialize  \tcode{exit_function} with \tcode{std::move(f)} otherwise initialize \tcode{exit_function} with \tcode{f}. 
...
\pnum
\effects If \tcode{EFP} is not an lvalue-reference type and \tcode{is_nothrow_constructible_v<EF,EFP>} is \tcode{true}, initialize  \tcode{exit_function} with \tcode{std::move(f)} otherwise initialize \tcode{exit_function} with \tcode{f}. If the initialization of \tcode{exit_function} throws an exception, calls \tcode{f()}.
...
\tcode{D} shall be CopyConstructible (Table 24
%\ref{copyconstructible}
) or \tcode{is_nothrow_move_constructible_v<D>} shall be \tcode{true}.
...
\tcode{R} shall be CopyConstructible (Table 24
%\ref{copyconstructible}
) or \tcode{is_nothrow_move_constructible_v<R>} shall be \tcode{true}.

\pnum
\begin{note} 
In case of \tcode{R} being an lvalue reference, an implementation should choose \tcode{reference_wrapper<R>} as type for \tcode{resource} and adjust functionality accordingly by delegating to \tcode{resource.get()} in appropriate places. 
\end{note}
...
\tcode{is_nothrow_constructible_v<R, RR>} \tcode{\&\&} \tcode{is_nothrow_constructible_v<D, DD>}.\\
...
\pnum
\effects If RR is not an lvalue-reference and \tcode{is_nothrow_constructible<R,RR>} is \tcode{true}, initializes \tcode{resource} with \tcode{std::move(r)}, otherwise initializes \tcode{resource} with \tcode{r}. Then, if \tcode{DD} is not an lvalue reference and \tcode{is_nothrow_constructible<D,DD>} is \tcode{true}, initializes  \tcode{deleter} with \tcode{std::move(d)}, otherwise initializes \tcode{deleter} with \tcode{d}. If initialization of \tcode{resource} throws an exception, calls \tcode{d(r)}.  If initialization of \tcode{deleter} throws an exception, calls \tcode{d(resource)}. 
\begin{note}
...
\remarks
The expression inside \tcode{noexcept} is equivalent to \\
\tcode{is_nothrow_move_constructible_v<R>} \tcode{\&\&} \tcode{is_nothrow_move_constructible_v<D>}.

\pnum
\effects If \tcode{is_nothrow_move_constructible_v<R>} is \tcode{true}, 
\begin{itemize}
\item
initialize \tcode{resource} from 
\tcode{std::forward<R>(rhs.resource)}, %should that be std::move ?
\item otherwise initialize \tcode{resource} from \tcode{rhs.resource}.  
\end{itemize}
...
\pnum
Then, 
if \tcode{is_nothrow_move_constructible_v<D>} is \tcode{true},
\begin{itemize}
\item initialize \tcode{deleter} from \tcode{std::forward<D>(rhs.deleter)};
\item otherwise initialize \tcode{deleter} from \tcode{rhs.deleter}. 
\end{itemize}

\pnum
If initialization of \tcode{deleter} throws an exception: 
\begin{itemize}
\item
if \tcode{!is_nothrow_move_constructible_v<R>}, 
\item
then \tcode{rhs.deleter(resource) ; rhs.release();} 
\item
otherwise \tcode{rhs.resource} and \tcode{rhs.deleter} are unmodified 
and \tcode{rhs} can be left owning the resource. 
\end{itemize}
...
\effects If \tcode{this == \&rhs} no effect. Otherwise,
\begin{itemize}
\item
\tcode{reset();} followed by
\item  
If \tcode{nothrow_move_assignable_v<R>}, 
\begin{itemize}
  \item try to copy or move assign \tcode{deleter} from \tcode{rhs.deleter} first,
  \item then \tcode{resource=std::forward<R>(rhs.resource)}, 
\end{itemize}
\item
else if  \tcode{nothrow_move_assignable_v<D>}, 
\begin{itemize}
  \item try to copy or move \tcode{resource} from \tcode{rhs.resource} first,
  \item then \tcode{deleter=std::forward<D>(rhs.deleter)}, 
\end{itemize}
\item
otherwise try to copy the two members. 
\item Then \tcode{execute_on_destruction = exchange(rhs.execute_on_destruction, false)}. 
\end{itemize}
\begin{note}
...
\pnum
\remarks The function \tcode{reset} shall not participate in overload resolution if the expression \tcode{resource = \textit{move_assign_if_noexcept}(r)} is ill-formed.
...
\end{codeblock}
If copy-assignment of \tcode{resource} throws an exception, \tcode{deleter(r)}.
...
\newpage
%\rSec2[scope.make_unique_resource]{Special factory for \tcode{unique_resource}}
\subsection {Special factory for \tcode{unique_resource} [scope.make_unique_resource]}
