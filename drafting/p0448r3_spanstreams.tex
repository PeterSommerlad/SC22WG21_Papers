...
namespace std {
  ...
  public:
    ...
    explicit basic_ispanstream(std::span<const charT> s)}
      :basic_ispanstream(
         std::span<charT>(const_cast<charT*>s.data(),s.size()){}\end{codeblock}\end{addedblock}\begin{codeblock}         
    ...
    void span(std::span<const charT> s) noexcept { 
       this->span(std::span<charT>(const_cast<charT*>s.data(),s.size())); 
...
\textbf{\emph{It is to be discussed in LEWG if such a change should also incorporate creating an \tcode{basic_ispanstream} from a \tcode{basic_string_view}.}
}
