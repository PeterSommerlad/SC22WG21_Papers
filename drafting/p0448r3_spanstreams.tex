...
\pnum
\constraints
\tcode{(not convertible_to<ROS, std::span<charT>{}>) \&\&}\\ \tcode{convertible_to<ROS, std::span<charT const>{}>} is \tcode{true}.

\pnum
\effects
Equivalent to:\\
\tcode{basic_ispanstream(std::span(const_cast<charT*>(data(s)),size(s))}.
\end{itemdescr}
\end{addedblock}
...
\effects
Equivalent to: \\
\tcode{this->span(std::span(const_cast<charT*>(data(s)),size(s)))}.
\end{itemdescr}
