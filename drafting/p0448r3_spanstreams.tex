...
\emph{Add the feature test macro \tcode{__cpp_lib_spanstream} to Table 17 ([tab:cpp.predefined.ft]) with the corresponding value for the header \tcode{\textless{}spanstream\textgreater}.}
...
The header \tcode{<spanstream>} defines four class templates and eight types that associate stream buffers with objects of class \tcode{span} as described in [views.span]. 
...
operation fails. Otherwise, the function determines \tcode{baseoff} as a value of type \tcode{off_type} as indicated in
...
If
\tcode{(baseoff + off) < 0},
or if \tcode{size_t(baseoff) + size_t(off)}  \tcode{ >= buf.size()},
or if \tcode{baseoff + off} would overflow, 
the positioning operation fails.
Otherwise, the function computes the value \tcode{newoff} of type \tcode{off_type} as \tcode{baseoff + off} and assigns
\tcode{xbeg + newoff}
...
\pnum
\effects
Equivalent to: \tcode{return seekoff(off_type(sp), ios_base::beg, which);}
\end{itemdescr}
...
\emph{Review question: can we construct \tcode{span(nullptr, 0)} ?}
...
\effects
Initializes the base class with
\tcode{basic_istream<charT, traits>(\&sb)}
and \tcode{sb} with
...
\pnum
\effects 
Initializes the base class from \tcode{std::move(rhs)} 
...
\pnum
\effects Equivalent to:\\
\tcode{return const_cast<basic_spanbuf<charT>*>(\&sb);}
\end{itemdescr}
...
\effects
Initializes the base class with
\tcode{basic_ostream<charT, traits>(\&sb)}
and \tcode{sb} with
...
\effects 
Initializes the base class from \tcode{std::move(rhs)} 
and \tcode{sb} from \tcode{std::move(rhs.sb)}.
...
\tcode{basic_iostream<charT, traits>(\&sb)}
...
\begin{itemdescr}
\pnum
\effects Initializes the base class from \tcode{std::move(rhs)} 
and \tcode{sb} from \tcode{std::move(rhs.sb)}.
%move constructing the base class, and the contained
%\tcode{basic_spanbuf}.
Next \tcode{basic_iostream<charT, traits>::set_rdbuf(\&sb)} is called to
install the contained \tcode{basic_spanbuf}.
\end{itemdescr}
