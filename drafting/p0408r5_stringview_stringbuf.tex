...
\item overload constructors with default arguments to only have the single argument version explicit.
\item introduce an exposition-only member function in \tcode{basic_stringbuf} to set the streambuf pointers (in the original version, that was part of the \tcode{str(string)} member function and now is needed in more than one place.
...
%% p0407: 
%\added{, and initializing \tcode{buf} with \tcode{\{s,a\}}}.
%\removed{Then calls \tcode{str(s)}.}
%\end{itemdescr}
...
%% p0407: 
\added{, and initializing \tcode{buf} with \tcode{\{std::move(s), a\}}}.
\end{itemdescr}
\end{insrt}
...
\pnum
\effects 
Move-assigns \tcode{buf} from \tcode{s}
%Moves the content of \tcode{s} into the \tcode{basic_stringbuf} underlying character sequence 
and initializes the input and output sequences according to \tcode{mode}.
...
\tcode{basic_istream(\&sb)}
and initializing \tcode{sb} with
\tcode{basic_stringbuf<charT, traits, Alloca\-tor>(str, which | ios_base::in)\added{, a})}~(\iref{stringbuf.cons}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Constructs an object of class \tcode{basic_istringstream<charT, traits, Allocator>}, initializing the base class with \tcode{basic_istream(\&sb)} and initializing \tcode{sb} with \tcode{basic_stringbuf<charT, traits, Allocator>(std::move(str), which | ios_base::in), a)}~(\iref{stringbuf.cons}).
\end{itemdescr}
\end{addedblock}
...
\tcode{sb}
with
\tcode{basic_stringbuf<charT, traits, Alloca\-tor>(which\added{, a})} ~(\iref{stringbuf.cons}).
\end{itemdescr}
...
and initializing \tcode{sb} with
\tcode{basic_stringbuf<charT, traits, Alloca\-tor>(str, which | ios_base::out\added{, a})}~(\iref{stringbuf.cons}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Constructs an object of class \tcode{basic_ostringstream<charT, traits, Allocator>}, initializing the base class with \tcode{basic_ostream(\&sb)} and initializing \tcode{sb} with \tcode{basic_stringbuf<charT, traits, Allocator>(std::move(str), which | ios_base::out, a)}~(\iref{stringbuf.cons}).
\end{itemdescr}
\end{insrt}
...
%      basic_string<charT, traits, Allocator>&& str,
%      const Allocator& a)
%      :basic_stringstream(std::move(str), ios_base::in | ios_base::out, a) {}
\end{addedblock}
\begin{codeblock}
...
\tcode{sb}
with
\tcode{basic_stringbuf<charT, traits, Alloca\-tor>(str, which\added{, a})}~(\iref{stringbuf.cons}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Constructs an object of class \tcode{basic_stringstream<charT, traits, Allocator>}, initializing the base class with \tcode{basic_stream(\&sb)} and initializing \tcode{sb} with \tcode{basic_stringbuf<charT, traits, Allocator>(std::move(str), which, a))}~(\iref{stringbuf.cons}).
\end{itemdescr}
\end{insrt}
