...
\item provide scope guard functionality through type \tcode{scope_exit} and \tcode{scope_guard} factory
...
\pnum
The header  \tcode{<scope_exit>} defines the class template \tcode{scope_exit} and the function template \tcode{scope_exit()} to create its instances.
...
\pnum
\effects constructs a scope_exit object that will call \tcode{deleter()} on its destruction if not \tcode{release()}ed prior to that. \tcode{execute_on_destruction} is set to \tcode{true}.
...
\pnum
\returns \tcode{scope_exit<D>(std::move(deleter))}
...
\pnum
\returns \tcode{unique_resource<R,D>(std::move(r), std::move(t),true);}
...
\pnum
\returns \tcode{unique_resource<R,D>(std::move(r), std::move(t), not bool(r==invalid);}
