...
\pnum
\effects 
If \tcode{is_nothrow_constructible_v<R,RR>} is \tcode{true}, initializes \tcode{resource} with \tcode{std::forward<RR>(r)}, otherwise initializes \tcode{resource} with \tcode{r}.
...
If initialization of \tcode{resource} throws an exception, calls \tcode{d(r)}.  If initialization of \tcode{deleter} throws an exception, calls \tcode{d(resource)}. 
\begin{note}
