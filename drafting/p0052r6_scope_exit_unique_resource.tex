...
\newcommand{\papernumber}{D0052r6}
\newcommand{\paperdate}{2017-11-08}
...
\tcode{is_same_v<uncvref_t<EFP>, scope_exit<EF>>} is \tcode{false}
...
\tcode{is_same_v<uncvref_t<EFP>, scope_fail<EF>>} is \tcode{false}
and \tcode{is_constructible_v<EF,EFP>} is \tcode{true}.
...
\pnum
\remarks This constructor shall not participate in overload resolution unless 
\tcode{is_same_v<uncvref_t<EFP>, scope_success<EF>>} is \tcode{false}
...
constexpr bool @\tcode{\textit{is_nothrow_move_or_copy_constructible_from_v}}@=
...
\pnum
\effects If RR is not an lvalue reference and \tcode{is_nothrow_constructible_v<R,RR>} is \tcode{true}, initializes \tcode{resource} with \tcode{std::move(r)}, otherwise initializes \tcode{resource} with \tcode{r}. Then, if \tcode{DD} is not an lvalue reference and \tcode{is_nothrow_constructible_v<D,DD>} is \tcode{true}, initializes  \tcode{deleter} with \tcode{std::move(d)}, otherwise initializes \tcode{deleter} with \tcode{d}. If initialization of \tcode{resource} throws an exception, calls \tcode{d(r)}.  If initialization of \tcode{deleter} throws an exception, calls \tcode{d(resource)}. 
\begin{note}
...
\begin{itemize}
\item If \tcode{is_nothrow_move_constructible_v<R>} is \tcode{true}, from \tcode{std::forward<R>(rhs.resource)}, 
...
from \tcode{std::forward<D>(rhs.deleter)};
\item otherwise, 
...
if \tcode{is_nothrow_move_constructible_v<R>} is \tcode{true}:
\begin{codeblock}
...
\begin{itemdescr}
\pnum
\requires \\\tcode{(is_nothrow_move_assignable_v<R> || is_copy_assignable_v<R>)} is \tcode{true}
and\\
...
\begin{itemdescr}
\pnum
\requires The expression \tcode{( resource == invalid ? true : false )} shall be well formed and have well defined behaviour.
...
\pnum
\effects Constructs and returns \tcode{unique_resource<decay_t<R>, decay_t<D>>\{std::forward<R>(resource), std::forward<D>(d)\}}. 
...
In that case, any failure during the construction of the return value will not call \tcode{d()}.
