...
\added{ p0052r5 introduced manipulators to allow both ways, but the default remains not to flush immediately.}
...
Users use the streambuf interface. Without access to the \tcode{basic_syncbuf} they would not be able call \tcode{emit()} on the underlying streambuf responsible the synchronization. Ask Pablo Halpern if you need more to be convinced.
...
The constructor for \tcode{osyncstream} takes a non-const reference to a \tcode{basic_ostream} obtaining its stream buffer or a \tcode{basic_streambuf} or can be put around such a stream buffer directly. This stream buffer indicates that the destruction of the \tcode{osyncstream} or calling its \tcode{emit()} member function may write to the stream buffer obtained with the constructor argument.
...
\item Should the manipulators be in header \tcode{<osyncstream>} instead of globally available?
\item Should the manipulator-hook member function of \tcode{basic_syncbuf} be public, or should the manipulators be made friends instead. However, without the accessor users can not directly force the stream buffer to pass through every sync.
...
\pnum
\effects
If \tcode{os} is a \tcode{basic_osyncstream<charT, traits, Allocator>} reference, calls \tcode{dynamic_cast<basic_osyncbuf<charT, traits, Allocator>*>(os.rdbuf())->set_emit_on_sync(true)}. Otherwise this manipulator has no effect. 
\begin{note}
...
\pnum
\effects
If \tcode{os} is a \tcode{basic_osyncstream<charT, traits, Allocator>} reference, calls \tcode{dynamic_cast<basic_osyncbuf<charT, traits, Allocator>*>(os.rdbuf())->set_emit_on_sync(false)}. Otherwise this manipulator has no effect. 
...
It defines a class template \tcode{basic_osyncstream} to buffer output and transfer the buffered content into an object of type \tcode{basic_streambuf<charT,traits>} atomically with respect to such transfers by other \tcode{basic_osyncstream<charT,traits,Allocator>} objects referring to the same \tcode{basic_streambuf<charT,traits>} object. The transfer occurs when \tcode{emit()} is called and when the \tcode{basic_osyncstream<chart,traits,Allocator>} object is destroyed.
...
\pnum
\effects
Constructs the \tcode{basic_syncbuf} object and sets \tcode{wrapped} to \tcode{obuf} which will be the final destination of characters inserted into the stream.

\remarks
If \tcode{obuf == nullptr}, output operations on \tcode{*this} will fail. A copy of \tcode{allocator} is used to allocate memory for internal buffers.
...
\pnum
\begin{note}
Subsequent calls to \tcode{emit()} might result in member functions being called on the user-provided stream buffer while a lock is held. 
\end{note}
...
\pnum
\effects
If \tcode{other.get_wrapped() == nullptr}, equivalent to \tcode{basic_syncbuf(nullptr, other.get_allocator())} otherwise the state necessary to perform \tcode{other.emit()} is moved to \tcode{*this}.
...
\pnum
\begin{note}
This constructor disassociates \tcode{other} from its wrapped stream buffer ensuring destruction of other produces no output. 
\end{note}
\end{itemdescr}
...
\pnum
\effects 
Calls \tcode{this->emit()}. If \tcode{rhs.get_wrapped() != nullptr}, the state necessary to perform \tcode{rhs.emit()} is moved to \tcode{*this}.
...
\pnum
\begin{note}
This assignment operator disassociates \tcode{rhs} from its wrapped stream buffer ensuring destruction of other produces no output. 
\end{note}
\end{itemdescr}
...
\pnum
\effects 
Atomically transfers the contents of the internal buffer to the stream buffer \tcode{*wrapped}, so that they appear in the output stream as a contiguous sequence of characters. If and only if a call was made to \tcode{this->sync()},  \tcode{wrapped->pubsync()} is called.

\pnum
\returns \tcode{true} if \tcode{wrapped != nullptr}, all of the buffered characters were successfully transferred, and the call to \tcode{wrapped->pubsync()} (if any) succeeded; otherwise \tcode{false}.
...
If \tcode{emit_on_sync == true}, calls \tcode{emit()}, otherwise
...
\pnum
\postconditions
The value returned by \tcode{this->get_wrapped()} is the value returned by \tcode{os.get_wrapped()} prior to calling this constructor. \tcode{nullptr == os.get_wrapped()}.
\end{itemdescr}
...
\pnum
\effects 
Calls \tcode{emit()} and swallows any exceptions arising.
\end{itemdescr}
...
\pnum
\effects 
First, calls \tcode{this->emit()} and swallows any exceptions arising. Move assigns \tcode{sb} from \tcode{other.sb}.
\begin{note}
This disassociates \tcode{other} from its wrapped stream buffer ensuring destruction of other produces no output. 
\end{note}

\pnum
\postconditions
Primarily, \tcode{nullptr == other.get_wrapped()}. Also, \tcode{get_wrapped()} returns the value previously returned by \tcode{other.get_wrapped()}.
\end{itemdescr}
