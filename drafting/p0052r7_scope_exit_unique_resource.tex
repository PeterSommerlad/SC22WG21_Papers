...
\begin{itemize}
\item remove the phrasing for potentially targeting a TS and minor editorial fixes.
...
\pnum
The constructor argument in the following constructors shall be a function object([function.objects]), 
lvalue reference to function, or 
...
The expressions
\tcode{d(r)} shall be well-formed, shall have well-defined behavior, and shall not throw an exception.
If \tcode{is_reference_v<R>} is true, \tcode{d(resource.get())} otherwise \tcode{d(resource)} shall be well-formed, shall have well-defined behavior, and shall not throw an exception.
...
\tcode{unique_resource\{std::forward<R>(resource), std::forward<D>(d)\}}. 
