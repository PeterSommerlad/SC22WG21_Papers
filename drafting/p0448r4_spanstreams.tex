...
\newcommand{\paperdate}{2021-02-26}
...
\begin{itemize}
\item Put all constraints on \tcode{ROS} in \constraints especially \tcode{ROS} models \tcode{ranges::borrowed_range}
\item explicitly convert to \tcode{std::span<const charT>} of ROS parameter.
\item use \tcode{span::data} \tcode{span::size} member functions.
...
\item fix the adopted wording (from \tcode{basic_stringbuf}) wrt seekoff by removing tables in favor of words.
...
\item added potential future change (not in the wording part) to provide \tcode{ispanstream} for \tcode{charT const*} arguments (this should be put in, before the next release, it is a kind of design bug fix.).
\end{itemize}
...
\item Why a separate \tcode{<spanstream>} header? why not just put it in one of the existing ones? 
...
\item Thanks to remote LWG meeting December 2020 and surrounding email feedback by Jeff Garland, Tim Song, Jens Maurer, Volle Voutilainen. Special thanks to Tim for pointing out the \tcode{const charT*} potential for \tcode{basic_ispanstream}.
\end{itemize}
...
\section{Future Directions (after p0448 is accepted for the WP: support \tcode{span<const charT>}}

During the finalizing review sessions Tim Song recognized a missing feature that \tcode{istrstream} supports, namely the use of the stream with a read-only input sequence (\tcode{char const*} in the case of \tcode{istrstream}). Some thinking it turned out that with a few additions an one change to class template \tcode{basic_ispanstream} this could be achieved, because an input only \tcode{basic_spanbuf} will never attempt to modify its underlying character sequence, because \tcode{pbackfail()} is not overridden.

With Tim Song's help the following extra constructor from a read-only \tcode{span<charT const>} was provided.


\removed{The following changes will be proposed either by a follow-up paper of if LEWG gives immediate blessing }
...
\end{codeblock}\end{addedblock}\begin{codeblock}         
    ...
    std::span<@\added{const }@charT> span() const noexcept;
...
\begin{addedblock}
\begin{note}
Constructing an \tcode{ispanstream} from a \fakegrammarterm{string-literal} will include the termination character \tcode{'\textbackslash{}0'} in the underlying \tcode{spanbuf}.
\end{note}
\end{addedblock}
...
\tcode{(!convertible_to<ROS, std::span<charT>{}>) \&\&}\\ \tcode{convertible_to<ROS, std::span<const charT>{}>} is \tcode{true}.

\pnum
\effects
Let \tcode{sp} be \tcode{std::span<const charT>(std::forward<ROS>(s))}. 
Equivalent to\\
\tcode{basic_ispanstream(std::span<charT>(const_cast<charT*>(sp.data()),sp.size()))}.
\end{itemdescr}
\end{addedblock}
...
Let \tcode{sp} be \tcode{std::span<const charT>(std::forward<ROS>(s)}). 
Equivalent to \\
\tcode{this->span(std::span<const charT>(const_cast<charT*>(sp.data()),sp.size()))}.
\end{itemdescr}
...
\pnum
The header \tcode{<spanstream>} defines class templates and types that associate stream buffers with objects whose types are specializations of \tcode{span} as described in [views.span]. 

\begin{note}
A user of theses classes is responsible for ensuring that the character sequence represented by the given \tcode{span} outlives the use of the sequence by objects of the classes in this subclause. Using multiple \tcode{basic_spanbuf} objects referring to overlapping underlying sequences from different threads, where at least one \tcode{basic_spanbuf} object is used for writing to the sequence, results in a data race.
\end{note}
...
\begin{itemize}
\item \tcode{(pptr() - pbase())} if \tcode{ios_base::out} is set in \tcode{mode} and \tcode{ios_base::in} is not set in \tcode{mode}, 
...
%%% LWG wants non textual spec

%    template<ranges::borrowed_range ROS>
%    requires (not convertible_to<ROS, std::span<charT>>) &&
%             convertible_to<ROS, std::span<charT const>>   
%    explicit basic_ispanstream(ROS&& s)
%      :basic_ispanstream(
%         std::span<charT>(const_cast<charT*>(data(s)),size(s)){}         
%    template<ranges::borrowed_range ROS>
%    requires (not convertible_to<ROS, std::span<charT>>) &&
%             convertible_to<ROS, std::span<charT const>>   
%    void span(ROS&& s) noexcept { 
%       this->span(std::span<charT>(const_cast<charT*>(data(s)),size(s))); 
%    }
...
\constraints
\tcode{ROS} models \tcode{ranges::borrowed_range}.\\
\tcode{(!convertible_to<ROS, std::span<charT>{}>) \&\&}\\ \tcode{convertible_to<ROS, std::span<charT const>{}>} is \tcode{true}.
...
Let \tcode{sp} be \tcode{std::span<const charT>(std::forward<ROS>(s))}. 
Equivalent to \\
\tcode{this->span(std::span<charT>(const_cast<charT*>(sp.data()), sp.size()))}.
\end{itemdescr}
