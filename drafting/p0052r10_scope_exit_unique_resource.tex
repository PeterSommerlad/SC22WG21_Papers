...
\newcommand{\papernumber}{p0052r10}
\newcommand{\paperdate}{2019-02-19}
...
\chapter{History}
\section{Changes from P0052R9}
\begin{itemize}
\item adjust clause numbers to n4800: 20->15, 23->19, and the table numbers for the Cpp17concepts as well. Note to editor: the latex contains the corresponding stable names in comments for most occurrences if not in the main text.
\item strike "Requires:" in non-item-description paragraphs putting requirements on class template arguments after the synopsis (for \tcode{scope_guard} and \tcode{unique_resource})
\item rephrase the use of "shall" in new style Mandates, Expects, and Constraints sections of a specification, by "is" or equivalent. (see also P1369).
\item add semicolon in front of otherwise in \emph{\tcode{scope_guard}} constructors Effects clause.
\item remove template argument from injected class name in Constraints clause of \emph{\tcode{scope_guard}} constructors.
\item rephrase Constraint clauses that use code into using words, like "is true".
\item add \tcode{unique_resource} default constructor and adjust specification as told by LWG.
\item adjust punctuation as instructed by LWG.
\item clarify saying "resource or deleter" in some places
\item remove superfluous paragraph number in longer Effects clauses.
\item rewrite Effects clause of \tcode{make_unique_resource_checked} as commonly phrased by LWG:

 "Effects: Returns an object constructed with
members initialized from std::forward<R>(resource), std::forward<D>(d),
and !bool(resource == valid).  Any failure during the construction of
the return value will not call d(resource) if bool(resource == invalid)
is true."

\begin{comment}
*Marshall*: Assuming we get a paper with these changes are we happy
moving it forward?  Do we need to look at the words again?  I instead
suggest that two volunteers verify that Peter made the changes, then move
it forward.  *Peter*: We are authorized to move it into the Library TS.
P1411 asks to move this paper into the IS.  *Marshall*: It goes into
the Library TS, and if there are no objections, it could go into C++20.
There are no LWG objections.  Peter must now check with LEWG on the
Library TS vs. C++20 decision.  *Daniel Sunderland* and *Jeff Garland*
volunteered.
\end{comment}
\item Added default constructor for \tcode{unique_resource} as proposed by p1411, to ease integration, once LEWG blesses it.\end{itemize}
...
\item introduce \textit{\tcode{RESOURCE}} in \tcode{unique_resource} specification to simplify wording. Note to reviewers \tcode{unique_resource::get()} does not need to use \textit{\tcode{RESOURCE}}, because \tcode{reference_wrapper} has an implicit conversion operator.
\item used "old concept" names from the current working paper, e.g., \oldconcept{Destructible}.
\item remove comma after "Throws Nothing,"
\item make \tcode{unique_resource} constructor non-explicit (it is not unary).
\item rename \tcode{execute_on_destruction} member \tcode{of unique_resource} to \tcode{execute_on_reset}.
\end{itemize}
Many thanks to Tim Song for his final walk over the text.
...
\item Thanks to LWG in Kona reviewing it and blessing it to forward. Especial thanks to Paul E. McKenney for taking splendid notes about the edits required and to Dan Sunderland and Jeff Garland for checking that I edited correctly.
\end{itemize}
\newpage
\chapter{Motivation}
...
\section{Adjust 15.5.5.12 Restrictions on exception handling [res.on.exception.handling]}
...
\section{Header}
In section 15.5.1.2 Headers [headers] add an entry to table 19 (cpp.library.headers) for the new header \tcode{<scope>}.

In section 15.5.1.3 Freestanding implementations [compliance] add an extra row to table 22 (cpp.headers.freestanding) and 
in section [utilities.general] add the same extra row to table 39 (util.lib.summary)
%%TODO clearer specification 
\begin{table}[htb]
\caption{table 22 (cpp.headers.freestanding) and table 39 (util.lib.summary)}
\begin{center}
\begin{tabular}{|lcl|}
\hline
&Subclause & Header\\
\hline
19.nn &Scope Guard Support & \tcode{<scope>}\\
\hline
\end{tabular}
\end{center}
\label{utilities}
...
Add a a new section to chapter 19 [utilities] introducing the contents of the header \tcode{<scope>}.
...
the requirements of \oldconcept{Destructible} (Table~30
%\ref{destructible}
). 
...
\pnum
\constraints
\tcode{is_same_v<remove_cvref_t<EFP>, scope_exit>} is \tcode{false}
and \\
\tcode{is_constructible_v<EF, EFP>} is \tcode{true}.
%\tcode{(not is_same_v<remove_cvref_t<EFP>, scope_exit<EF>{}>} \tcode{\&\&}\\
%\tcode{is_constructible_v<EF, EFP>)}.

\pnum
\mandates % well-defined behavior not good here. 
%Given an lvalue \tcode{f} of type \tcode{remove_reference_t<EFP>}, t
The expression \tcode{f()} is well-formed.

\pnum
\expects
Calling \tcode{f()} has well-defined behavior and does not throw an exception.

\pnum
\effects If \tcode{EFP} is not an lvalue reference type and \tcode{is_nothrow_constructible_v<EF,EFP>} is \tcode{true}, 
initialize  \tcode{exit_function} with \tcode{std::forward<EFP>(f)}; 
otherwise initialize \tcode{exit_function} with \tcode{f}. 
...
\pnum
\constraints  
\tcode{is_same_v<remove_cvref_t<EFP>, scope_fail>} is \tcode{false}
and \\
\tcode{is_constructible_v<EF,EFP>} is \tcode{true}.
%\tcode{(not is_same_v<remove_cvref_t<EFP>, scope_fail<EF>{}>} \tcode{\&\&}\\\tcode{is_constructible_v<EF,EFP>)}.
...
\pnum
\constraints  
\tcode{is_same_v<remove_cvref_t<EFP>, scope_success>} is \tcode{false}
and \\
\tcode{is_constructible_v<EF,EFP>} is \tcode{true}.
%\tcode{(not is_same_v<remove_cvref_t<EFP>, scope_success<EF>{}>} \tcode{\&\&}\\
%\tcode{is_constructible_v<EF,EFP>)}.
...
\pnum
\expects
Calling \tcode{f()} has well-defined behavior.
...
\begin{itemdescr}
\pnum
\requires % we use old concept names
If \tcode{EF} is an object type:
\begin{itemize}
 \item
if \tcode{is_nothrow_move_constructible_v<EF>} is true, \tcode{EF} shall meet the requirements of \oldconcept{MoveConstructible} (Table 26), 
\item
otherwise \tcode{EF} shall meet the requirements of \oldconcept{CopyConstructible} (Table 27).
\end{itemize}

%%% the following would get rid of requires clause above:
%\expects
%If \tcode{EF} is an object type:
%\begin{itemize}
% \item
%if \tcode{is_nothrow_move_constructible_v<EF>} is true, \tcode{EF} meets the requirements of \\\oldconcept{MoveConstructible} (Table 26), 
%\item
%otherwise \tcode{EF} meets the requirements of \oldconcept{CopyConstructible} (Table 27).
%\end{itemize}
...
%\rSec3[scope.unique_resource.class]{Class template \tcode{unique_resource}}
\subsection {Class template \tcode{unique_resource} [scope.unique_resource.class]}
%% Note default ctor blessed by LEWG?
\begin{codeblock}
template <class R,class D>
class unique_resource {
public:
...
\tcode{D} shall meet the requirements of a 
...
\tcode{D} shall either meet the \oldconcept{CopyConstructible} requirements (Table 27
%\ref{copyconstructible}
), or 
\tcode{D} shall meet the \oldconcept{MoveConstructible} requirements (Table 26
...
a \emph{resource type} \tcode{T} is an object type that meets the
\oldconcept{CopyConstructible} (Table 27
%\ref{copyconstructible}
) requirements,  or 
is an object type that meets the \oldconcept{MoveConstructible} (Table 26
%\ref{moveconstructible}
) requirements and
...
\pnum
For the scope of this clause let \tcode{\textit{RESOURCE}} be defined as follows:
\begin{itemize}
\item \tcode{resource.get()} if \tcode{is_reference_v<R>} is \tcode{true},
\item \tcode{resource} otherwise.
\end{itemize}
...
\tcode{(is_nothrow_constructible_v<D , DD> || is_constructible_v<D ,DD\&>)}
% is \tcode{true}
.\\
\begin{note}
...
\mandates
The expression
\tcode{d(r)}, \tcode{d(\textit{RESOURCE})} and \tcode{deleter(\textit{RESOURCE})} is well-formed.

\pnum
\expects
Calling \tcode{d(r)}, \tcode{d(\textit{RESOURCE})} or \tcode{deleter(\textit{RESOURCE})} has well-defined behavior and does not throw an exception. 
...
If initialization of \tcode{resource} throws an exception, calls \tcode{d(r)}. If initialization of \tcode{deleter} throws an exception, calls \tcode{d(\textit{RESOURCE})}.
\begin{note}
The explained mechanism ensures no leaking of resources.
\end{note}

\pnum
\throws Any exception thrown during initialization of \tcode{resource} or \tcode{deleter}.
...
and \tcode{rhs.execute_on_reset} is \tcode{true}: %% bugfix in R8
\begin{codeblock}
    rhs.deleter(@\textit{RESOURCE}@);
    rhs.release();
\end{codeblock}

Finally, \tcode{execute_on_reset} is initialized with \tcode{exchange(rhs.execute_on_reset,false)}.
...
\begin{itemdescr}
\pnum
\requires %\mandates not useful for oldconcept
If \tcode{is_nothrow_move_assignable_v<R1>} is \tcode{true}, \tcode{R1} shall meet the \oldconcept{MoveAssignable} requirements (Table 28); otherwise \tcode{R1} shall meet the \oldconcept{CopyAssignable} (Table 29) requirements.
If \tcode{is_nothrow_move_assignable_v<D>} is \tcode{true}, \tcode{D} shall meet the \oldconcept{MoveAssignable} requirements (Table 28); otherwise \tcode{D} shall meet the \oldconcept{CopyAssignable} requirements (Table 29).

%\expects %\mandates not useful for oldconcept
%If \tcode{is_nothrow_move_assignable_v<R1>} is \tcode{true}, \tcode{R1} meets the \oldconcept{MoveAssignable} requirements (Table 28); otherwise \tcode{R1} meets the \oldconcept{CopyAssignable} (Table 29) requirements.
%If \tcode{is_nothrow_move_assignable_v<D>} is \tcode{true}, \tcode{D} meets the \oldconcept{MoveAssignable} requirements (Table 28); otherwise \tcode{D} meets the \oldconcept{CopyAssignable} requirements (Table 29).
...
%
%\pnum
%Then \tcode{execute_on_reset = exchange(rhs.execute_on_reset, false)}. 
...
\begin{itemdescr}
\pnum
\constraints The selected assignment expression statement assigning \tcode{resource} is well-formed.

\pnum
\mandates
The expression \tcode{deleter(r)} is well-formed.

\pnum
\expects
Calling \tcode{deleter(r)} has well-defined behavior and does not throw an exception.
...
\pnum
\effects 
Equivalent to \tcode{execute_on_reset = false}.
\end{itemdescr}
...
\pnum
\constraints 
\tcode{is_pointer_v<R>} is \tcode{true} and \tcode{is_void_v<remove_pointer_t<R>{}>} is \tcode{false}.
%\tcode{(is_pointer_v<R> \&\& !is_void_v<remove_pointer_t<R>{}>)}
% is \tcode{true}
...
\pnum
\constraints 
\tcode{is_pointer_v<R>} is \tcode{true}.
%is \tcode{true}
. 
...
\begin{itemdescr}
\pnum
\mandates The expression \tcode{( resource == invalid ? true : false )} is well-formed.

\pnum
\expects Evaluation of the expression \tcode{( resource == invalid ? true : false )} has well-defined behavior and does not throw an exception.
...
\pnum
\effects 
Returns an object constructed with members initialized from \tcode{std::forward<R>(resource)}, \tcode{std::forward<D>(d)}, and \tcode{!bool(resource == invalid)}. Any failure during construction of the return value will not call \tcode{d(resource)} if \tcode{bool(resource == invalid)} is \tcode{true}.

%Constructs and returns \\
%\tcode{\{std::forward<R>(resource), std::forward<D>(d)\}}. 
%
%\pnum
%If \tcode{bool(resource == invalid)} evaluates to \tcode{true} before the construction, the returned \tcode{unique_resource} object's \tcode{execute_on_reset} is \tcode{false}. 
