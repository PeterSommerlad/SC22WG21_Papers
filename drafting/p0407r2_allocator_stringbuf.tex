...
\newcommand{\paperdate}{2017-11-10}
...
\begin{itemize}
\item Does it make sense to add noexcept specifications for move and swap, since the base classes do not. At least it does not make sense so for stream objects, since the base classes do not specify that.
...
\tcode{basic_streambuf()}\iref{streambuf.cons}, \removed{and} initializing
\tcode{mode}
with \tcode{which}\added{, and \tcode{buf} with \tcode{\{s,a\}}}.
...
\added{In the second form \tcode{buf} is initialized from \tcode{\{std::move(rhs.buf),a\}}.}
It
is
...
\section{30.8.2.3 Assign and swap [stringbuf.assign]}
