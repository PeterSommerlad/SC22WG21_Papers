...
\markboth{\NNumber \DocDate}{\NNumber \DocDate}
...
\date{\DocDate}                                           % Activate to display a given date or no date
\input{macros}
...
\begin{itemize}
\item Rid remains of \tcode{invoke_it} enumeration that was missed in the removal.
...
\item removed \tcode{unique_resource::operator*()}, because it is superfluous (agreed upon by LEWG on Saturday morning in Urbana).
\end{itemize}
...
\begin{itemize}
\item Could \tcode{unique_resource::operator*() }be removed, because type conversion operator is sufficient to get the original \tcode{operator*} on the resource, if it is pointer-like.
\end{itemize}
...
\pnum
\effects Calls \tcode{exit_function()} if not previously \tcode{release()}ed.
...
\end{codeblock}
% 	@\seebelow@ operator*() const; // Note by author: might be superfluous!
...
\pnum
\effects constructs a \tcode{unique_resource} by moving \tcode{resource} and then \tcode{deleter}. The constructed object will call \tcode{deleter(resource)} on its destruction if \tcode{shouldRun} is \tcode{true} and not \tcode{release()}ed prior to destruction. \tcode{execute_on_destruction} takes the value of \tcode{shouldRun}.
...
%\textit{\\Note by the author:} defining \tcode{operator*} seems superfluous, because we have automatic type conversion.
%\\
%
%\begin{itemdecl}
...
%% * is also available through conversion operator that is non-explicit...
